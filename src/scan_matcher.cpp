...
>>>>>>> a2fc9fa50a457f043cfe830bfe723262ca3c6c73
...
>>>>>>> a2fc9fa50a457f043cfe830bfe723262ca3c6c73
  ...
  for (uint64_t k = 0; k < graph.poses.size(); k++) {
    ...
    this->final_poses.emplace_back(
        ros_data->getLidarScanTimePoint(this->pose_scan_map[k]), 0, temp_trans);
  }
  }
...
>>>>>>> a2fc9fa50a457f043cfe830bfe723262ca3c6c73
      ...
      for (uint64_t iter = 0; iter < ros_data->lidar_container.size(); iter++) {
        ...
        case 1:
          try {
            ...
            T_ECEF_GPS =
                gpsToEigen(gps_pose.first, true); // true: apply T_ENU_GPS
        ...
        case 2:
          try {
            ...
            LOG_INFO(
                "No odometry message for time of scan, may happen at edges of "
                "recorded data");
            use_next_scan = true;
            break;
          }
        ...
        if (i > 0) {
          ...
          if (take_new_scan && !use_next_scan) {
            this->init_pose.emplace_back(ros_data->getLidarScanTimePoint(iter),
                                         0, T_MAP_LIDAR);
    ...
    void ScanMatcher::findAdjacentScans() {
      ...
      for (uint64_t j = 0; j < this->init_pose.size(); j++) {
         ...
         */
        for (uint16_t k = j + 2;
             (k < j + 1 + this->params.knn) && (k < this->init_pose.size());
             k++) {
    ...
    void ScanMatcher::createAggregateMap(boost::shared_ptr<ROSBag> ros_data,
      ...
      for (uint64_t k = 0; k < this->final_poses.size(); k++) {
        ...
        TimePoint curr_pose_time =
            ros_data->lidar_container[curr_index].time_point;
        int next_index;
        ...
        case 1:
          ...
          pcl::transformPointCloud(
              *(ros_data->lidar_container[curr_index].value),
          ...
          if ((this->params.trajectory_sampling_dist >
            ...
            while (true) {
              ...
              T_MAP_LLOC_J.matrix() = interpolateTransform(
                  T_MAP_LLOC_k.matrix(), curr_pose_time,
                  T_MAP_LLOC_kp1.matrix(), next_pose_time, time_point_J);

              bool take_new_map_scan =
                  takeNewScan(T_MAP_LLOC_Jprev, T_MAP_LLOC_J,
                              this->params.map_sampling_dist,
          ...
          if ((this->params.trajectory_sampling_dist >
            ...
            while (true) {
              ...
              T_MAP_LMAP_J.matrix() = interpolateTransform(
                  T_MAP_LMAP_k.matrix(), curr_pose_time,
                  T_MAP_LMAP_kp1.matrix(), next_pose_time, time_point_J);
              bool take_new_map_scan =
                  takeNewScan(T_MAP_LMAP_Jprev, T_MAP_LMAP_J,
                              this->params.map_sampling_dist,
                              this->params.map_rotation_change);
    ...
    void ScanMatcher::outputAggregateMap(int mapping_method,
                                         std::string path_) {
      if (this->params.downsample_output_method == 3) {
        *this->aggregate = downSampleFilterIG(
            this->aggregate, this->params.downsample_cell_size);
      }
      std::string dateandtime =
          convertTimeToDate(std::chrono::system_clock::now());
      std::string mapType;
      ...
      pcl::io::savePCDFileBinary(path_ + dateandtime + mapType,
                                 *this->aggregate);
    ...
    bool ICPScanMatcher::matchScans(
      ...
      auto T_MAP_Lj =
          init_pose.at(j).value; // set initial guess of current scan
      auto T_MAP_Li =
          init_pose.at(i).value; // set initial guess of adjacent scan
      ...
      *cloud_tgt2 = *ros_data->lidar_container[pose_scan_map.at(i)].value;
      ...
      if (matcher.match()) {
        auto T_estLj_Lj =
            this->matcher.getResult(); // assign estimated transform to new
                                       // current position
