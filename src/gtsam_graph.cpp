...
void GTSAMGraph::addFactor(uint64_t from,
                           uint64_t to,
                           ...
                           const wave::Mat6 &info)
{
    ...
    this->graph.add(gtsam::BetweenFactor<gtsam::Pose3>(id1, id2, pose, noise_model));
}
...
void GTSAMGraph::optimize()
{
    ...
    gtsam::LevenbergMarquardtOptimizer optimizer(this->graph, this->initial, params);
    this->result.clear();
